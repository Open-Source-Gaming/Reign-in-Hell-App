...
- [x] Describe the project in one sentence.
...
- [x] What type of project is this? (e.g., web app, API, SaaS, monorepo, mobile backend, CLI tool, library, other)
...
- [x] If this is a monorepo, what are the workspace names and their purposes?
...
- [x] Who are the target users? (e.g., developers, end consumers, internal team, B2B customers)
- **Destination files**: `docs/REQUIREMENTS.md`, `README.md`
- **Validation**: At least one user type with a brief description.

### Q3.2: Business goal

- [x] What is the primary business goal? What problem does this solve?
...
- [x] What is the primary programming language?
...
- [x] What framework(s) will be used? (e.g., Next.js, Express, FastAPI, Django, Rails)
- **Destination files**: `CLAUDE.md`, `replit.md`, `docs/ARCHITECTURE.md`
- **Validation**: Specific framework with version if known.

### Q4.3: Runtime

- [x] What runtime environment? (e.g., Node.js 20, Python 3.12, Go 1.22)
- **Destination files**: `CLAUDE.md`, `replit.md`, `docs/ENVIRONMENT.md`
- **Validation**: Runtime with minimum version.

### Q4.4: Database

- [x] What database(s) will be used? (e.g., PostgreSQL, MongoDB, SQLite, none)
...
- [x] What package manager? (e.g., npm, pnpm, yarn, pip, poetry, cargo)
- **Destination files**: `CLAUDE.md`, `replit.md`, `docs/ENVIRONMENT.md`
- **Validation**: Must be specific.

### Q5.2: Test framework

- [x] What test framework? (e.g., Jest, Vitest, pytest, Go testing)
- **Destination files**: `docs/TESTING.md`, `.claude/rules/testing.md`
- **Validation**: Must be specific.

### Q5.3: Test file location

- [x] Where should test files live? (co-located with source, or in a separate `tests/` directory)
- **Destination files**: `docs/TESTING.md`, `.claude/rules/testing.md`
- **Validation**: One of the two options.

### Q5.4: Coverage target

- [x] What is the target code coverage percentage? (e.g., 80%, or "no hard target")
...
- [x] Where will this be deployed? (e.g., Vercel, AWS, GCP, Fly.io, Heroku, self-hosted, Replit)
- **Destination files**: `docs/DEPLOYMENT.md`, `docs/ENVIRONMENT.md`, `replit.md`
- **Validation**: Specific platform.

### Q6.2: CI/CD

- [x] What CI/CD system? (e.g., GitHub Actions, GitLab CI, none yet)
...
- [x] What level of feature documentation granularity? (full: all 8 sub-files per feature, standard: overview + requirements + tests, minimal: overview only)
...
- [x] Are there any scaffolded doc files that are definitely not needed? (e.g., MIGRATIONS.md if no database, API.md if no API)
...
- [x] Which scaffolded files should be filled in immediately? (beyond those auto-populated from other answers)
...
- [x] What is the planning source of truth? (e.g., Notion, Linear, GitHub Issues, this repo only)
- **Destination files**: `docs/SYNC.md`, `CLAUDE.md`, `.mcp.json`
- **Validation**: Specific tool.

### Q9.2: Notion workspace

- [x] If Notion, what is the workspace name and relevant database IDs?
...
- [x] How often should planning sync occur? (e.g., before each feature, daily, on-demand)
...
- [x] What is the primary LLM CLI? (e.g., Claude Code, Replit Agent, both)
- **Destination files**: `CLAUDE.md`, `replit.md`, `AGENTS.md`
- **Validation**: Must be specific.

### Q10.2: Approval requirements

- [x] What actions require explicit approval? (defaults: commits, pushes, destructive changes, dependency additions)
...
- [x] What actions can the LLM CLI take without asking? (e.g., read files, run tests, lint, format)
...
- [x] Any project-specific writing rules beyond the defaults? (defaults: no em dashes, imperative voice, direct language)
- **Destination files**: `.claude/rules/safety.md`, `.claude/rules/code-style.md`
- **Validation**: List of rules or "use defaults".

### Q11.2: Coding rules

- [x] Any project-specific coding rules beyond the defaults? (e.g., no classes, functional only, specific patterns)
- **Destination files**: `.claude/rules/code-style.md`, `CLAUDE.md`
- **Validation**: List of rules or "use defaults".

### Q11.3: Banned patterns

- [x] Are there any patterns, libraries, or approaches that are explicitly banned?
...
- [x] Does this project need a web app to display content (blog posts, articles, project pages)?
- **Destination files**: `CLAUDE.md`, `docs/ARCHITECTURE.md`
- **Validation**: yes or no.

### Q12.2: Static site generation

- [x] Should the web app be a static site (built ahead of time, no server-side rendering at request time)?
- **Destination files**: `docs/ARCHITECTURE.md`, `CLAUDE.md`
...
- [x] Which static site generator? (Astro, Hugo, Eleventy, Next.js static export, other)
...
- [x] Does this project need a headless CMS for content authoring? (none, Ghost, Strapi, Contentful, WordPress, other)
...
- [x] What is the production domain? Used to derive `ghost.<domain>` and Worker route.
...
- [x] What container prefix should Ghost services use? (e.g., `myproject` -> `myproject-ghost`, `myproject-db`)
...
- [x] Should Ghost be protected behind Cloudflare Tunnel and Cloudflare Access? (recommended for production)
